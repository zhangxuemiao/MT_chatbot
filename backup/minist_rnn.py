...
def RNN(_X, _istate, _weights, _biases):
    ...
    b = tf.cast(_biases['hidden'], tf.int32)
    w = tf.cast(_weights['hidden'], tf.int32)
    x = tf.cast(_X, tf.int32)
    _X = tf.matmul(x, w) + b
    # 之后使用LSTM
    ...
    outputs, states = tf.nn.dynamic_rnn(lstm_cell, _X, initial_state=_istate)
